...
for i in range(1501):
    N[i]+=SNR(z0[i])
    lz[i]=0.8*N[i]/(1+z0[i])
    nr[i]=0.43633*(4/3)*np.pi*(a.Dcofz(z0[i]+0.001)**3-(a.Dcofz(z0[i]))**3)*SNR(z0[i])/(1+z0[i]) #dilation necessary?
    n+=nr[i]    
...
plt.xlabel('redshift, $z$',size=12)
plt.ylabel('number of SNe Ia per year',size=12)
